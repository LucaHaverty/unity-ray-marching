Shader "Hidden/RayMarching"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float SmoothMin(float3 pointPos, Cube objA, Sphere objB)
            {
                float a = CubeSDF(pointPos, objA);
                float b = SphereSDF(pointPos, objB);
                float k = .4;
            ...
            {
                pointPos = float3(abs(pointPos.x % 1), abs(pointPos.y % 1), abs(pointPos.z % 1));
